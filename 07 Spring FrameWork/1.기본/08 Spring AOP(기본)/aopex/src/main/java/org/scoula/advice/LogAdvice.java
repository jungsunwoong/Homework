...
public class LogAdvice {
    ...
    //어떤 타겟이 되는 클래스의 메서드를 호출해서 실행할 떄
    ...
    //부가적인 기능을 넣을 TARGET 이 되는 sampleservice 클래스 안에 있는
    //조인포인트가 되는 모든 메소드를 호출해서 실행하기 전에, (이벤트라고 인식함)
    //logBefore() 메서드를 먼저 호출해라! 설정
    // ==> 이벤트가 발생하면 aop 프락시 객체가 logBefore() 메서드를 먼저 호출함.
    // 스프링에서 사용된 설계 기법 3가지 => 24~26개 설계 패턴
    // 프락시(대리인) 설계 기법, design pattern
    // 싱글톤 패턴
    // 팩토리
